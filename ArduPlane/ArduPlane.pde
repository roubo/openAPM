 ...
 */
/*在APM飞控系统中，采用的是两极PID控制方式，第一级是导航级，第二级是控制级，导航级的计算集	中在medium_loop()和fastloop()的update_current_flight_mode()函数中，控制级集中在fastl	oop的stabilize()函数中。导航级PID控制就是要解决飞机如何一预定空速飞行在预定高度的问题	，以及如何转弯飞往目标问题，通过算法给出飞机需要的俯仰角、油门和横滚角，然后交给控制级进	行控制计算。控制级的任务就是依据需要的俯仰角、横滚角油门，结合飞机当前的姿态计算出合适的舵机控制量，使飞机保持预定的俯仰角、横滚角和方向角。最后通过舵机控制级set_servos_4()将控制量转换成具体的pwm信号量输出给舵机。值得一提的是，油门的控制量是在导航级确定的。控制级中不对油门控制量进行计算，而直接交给舵机控制级。而对于方向舵的控制，导航级并不给出方向舵量的计算，而是由控制级直接计算方向舵控制量，然后再交给舵机控制级。  */
////////////////////////////////////////////////////////////////////////////////
// Header includes
////////////////////////////////////////////////////////////////////////////////
